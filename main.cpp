...
void CloseActiveTabAndMinimize(HWND hBrowser) {
    if (GetForegroundWindow() == hBrowser) {
        ...
        Sleep(150); // Increased for reliability
    }
...
private:
    ...
    void setupListsPage() {
        ...
        auto makeList = [&](QString title, QLineEdit*& inA, QComboBox*& inW, QListWidget*& lA, QListWidget*& lW, int col) {
            ...
            connect(bA, &QPushButton::clicked, [=](){
                QString text = inA->text().trimmed().toLower();
                if(!text.isEmpty()){
                    if(!text.endsWith(".exe")) text += ".exe";
                    lA->addItem(text); inA->clear(); SyncListsFromUI(); 
                } 
            });
            connect(bW, &QPushButton::clicked, [=](){
                QString text = inW->currentText().trimmed().toLower();
                if(!text.isEmpty()){
                    lW->addItem(text); inW->setCurrentText(""); SyncListsFromUI(); 
                } 
            });
    ...
    void setupTray() {
        ...
        if(QFile::exists(iconPath)) {
            ...
            // Auto-generated fallback icon to guarantee Tray visibility
            QPixmap pix(32, 32);
            ...
            p.drawText(pix.rect(), Qt::AlignCenter, "RF");
            icon = QIcon(pix);
        }
