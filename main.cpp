...
bool blockAdult = true; // ADULT BLOCK ALL TIME DEFAULT ON
...
// Registry Auto Start (No Admin needed)
...
protected:
    ...
    void paintEvent(QPaintEvent *e) override {
        ...
        p.fillPath(path, isChecked() ? QColor("#1CB8C9") : QColor("#CBD5E1")); // CareUEyes Bright Cyan
...
public:
    ...
    CircularProgress(QWidget *parent = nullptr) : QWidget(parent) { setMinimumSize(300, 300); }
...
protected:
    ...
    void paintEvent(QPaintEvent *) override {
        ...
        QPen bgPen(QColor("#E2E8F0"), 18); p.setPen(bgPen); p.drawArc(rect, 0, 360 * 16);

        QPen progPen(QColor("#1CB8C9"), 18); progPen.setCapStyle(Qt::RoundCap); p.setPen(progPen);

        int spanAngle = -(progress * 360 * 16) / 100; p.drawArc(rect, 90 * 16, spanAngle);

        p.setPen(QColor(isDarkMode ? "#F8FAFC" : "#0F172A")); p.setFont(QFont("Segoe UI", 28, QFont::Bold)); p.drawText(rect, Qt::AlignCenter, centerText);
...
public:
    ...
    StopwatchWindow() {
        ...
        bgFrame->setStyleSheet("background-color: rgba(30, 41, 59, 0.90); border-radius: 15px; border: 3px solid #1CB8C9;");
...
public:
    ...
    void applyTheme() {
        ...
        QString baseStyle = QString(R"(
            ...
            QLabel, QCheckBox, QRadioButton { color: %3; font-size: 18px; font-family: 'Segoe UI', Arial, sans-serif; }
    ...
    QWidget* createSidebarItem(QString iconPath, QString text) {
        ...
        label->setStyleSheet("font-weight: bold; font-size: 20px; background: transparent; color: inherit;"); 
    ...
    RasFocusApp() {
        ...
        QHBoxLayout* tbLayout = new QHBoxLayout(titleBar); tbLayout->setContentsMargins(25, 0, 0, 0); tbLayout->setSpacing(0);

        

        QLabel* appTitle = new QLabel("👁️ RasFocus Pro"); appTitle->setStyleSheet("font-weight: bold; color: #0F172A; font-size: 18px; border: none;");

        tbLayout->addWidget(appTitle); tbLayout->addStretch();

        

        QPushButton* btnMin = new QPushButton("—"); btnMin->setFixedSize(60, 50);

        btnMin->setStyleSheet("QPushButton { border: none; background: transparent; color: #64748B; font-weight: bold; border-radius:0px; font-size: 20px; min-height: 50px; } QPushButton:hover { background: #E2E8F0; color: #0F172A; }");

        connect(btnMin, &QPushButton::clicked, this, &QWidget::showMinimized); tbLayout->addWidget(btnMin);

        

        QPushButton* btnClose = new QPushButton("X"); btnClose->setFixedSize(60, 50);

        btnClose->setStyleSheet("QPushButton { border: none; background: transparent; color: #64748B; font-weight: bold; border-radius:0px; font-size: 20px; min-height: 50px;} QPushButton:hover { background: #EF4444; color: white; }");
        ...
        sidebar->setStyleSheet(R"(
            ...
            QListWidget::item:selected QLabel { color: #1CB8C9; }
...
private:
    ...
    void setupListsPage() {
        ...
        QFrame* modeCard = createCard(); QHBoxLayout* radL = new QHBoxLayout(modeCard); radL->setContentsMargins(30, 25, 30, 25);

        rbBlock = new QRadioButton("Block List Mode"); rbAllow = new QRadioButton("Allow List Mode (Strict)");

        rbBlock->setStyleSheet("font-size: 20px; font-weight:bold;"); rbAllow->setStyleSheet("font-size: 20px; font-weight:bold;");
        ...
        QFrame* listCard = createCard(); QGridLayout* g = new QGridLayout(listCard); g->setContentsMargins(40, 40, 40, 40); g->setSpacing(25);

        QString btnStyle = "background-color: #1CB8C9; color: white; font-weight: bold; padding: 0 30px;";

        QString removeBtnStyle = "background-color: #64748B; color: white; padding: 15px; border-radius: 8px;";
        ...
        auto makeList = [&](QString title, QLineEdit*& inA, QComboBox*& inW, QListWidget*& lA, QListWidget*& lW, int col) {
            ...
            lA = new QListWidget(); lA->setStyleSheet("border: 2px solid #E2E8F0; background: transparent; padding: 10px; font-size: 18px;"); g->addWidget(lA, 2, col); QPushButton* rA = new QPushButton("Remove"); rA->setStyleSheet(removeBtnStyle); g->addWidget(rA, 3, col);
            ...
            lW = new QListWidget(); lW->setStyleSheet("border: 2px solid #E2E8F0; background: transparent; padding: 10px; font-size: 18px;"); g->addWidget(lW, 6, col); QPushButton* rW = new QPushButton("Remove"); rW->setStyleSheet(removeBtnStyle); g->addWidget(rW, 7, col);
        ...
        listRunning = new QListWidget(); listRunning->setStyleSheet("border: 2px solid #E2E8F0; background: transparent; padding: 10px; font-size: 18px;"); g->addWidget(listRunning, 1, 1, 6, 1);
    ...
    void setupToolsPage() {
        ...
        QFrame* topFrame = new QFrame(); topFrame->setStyleSheet("background-color: #1CB8C9;"); topFrame->setFixedHeight(350);

        QVBoxLayout* tL = new QVBoxLayout(topFrame); tL->setAlignment(Qt::AlignCenter); tL->setSpacing(15);

        QLabel* title = new QLabel("Next break in:"); title->setStyleSheet("color: white; font-weight: bold; font-size: 24px;"); title->setAlignment(Qt::AlignCenter);

        lblPomoTime = new QLabel("00:00:00"); lblPomoTime->setStyleSheet("color: white; font-size: 120px; font-family: 'Segoe UI Light';"); lblPomoTime->setAlignment(Qt::AlignCenter);

        bPStart = new QPushButton("START"); bPStart->setFixedSize(280, 70); bPStart->setStyleSheet("background-color: #FF4D4D; color: white; font-weight: bold; font-size: 26px; border-radius: 35px;");
        ...
        bPStop = new QPushButton("Stop Timer"); bPStop->setStyleSheet("background: transparent; color: #EF4444; text-decoration: underline; font-weight: bold; font-size: 20px;"); bL->addWidget(bPStop, 0, Qt::AlignCenter);

        lblPomoStatus = new QLabel("Ready"); lblPomoStatus->setAlignment(Qt::AlignCenter); lblPomoStatus->setStyleSheet("font-size: 20px; font-weight:bold; color: #64748B;"); bL->addWidget(lblPomoStatus);
...
int main(int argc, char *argv[]) {
    ...
    // 1. Desktop Shortcut Creation on First Run

    CreateDesktopShortcut();



    // 2. Single Instance Check & Bring to Front (Works without Admin prompt)
