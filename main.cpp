...
private:
                ...
                if(isSessionActive) {
                    ...
                    if(isBrowser) {
                        if(useAllowMode) {
                            ...
                            if(!ok && !sTitle.contains("allowed websites")) { CloseActiveTabAndMinimize(hActive); 
                                QString p = QDir::currentPath() + "/allowed_sites.html"; QFile f(p); if(f.open(QIODevice::WriteOnly)){ QTextStream out(&f); out<<"<html><body><h2>Allowed:</h2>"; for(auto x:allowedWebs) out<<"<a href='https://"<<x<<"'>"<<x<<"</a><br>"; out<<"</body></html>"; f.close(); } QDesktopServices::openUrl(QUrl::fromLocalFile(p)); 
                            }
                        } else {
    ...
    void slowLoop() { // 1000ms
        if(isSessionActive) {
            ...
            if(isPomodoroMode) {
                ...
                if(!isPomodoroBreak && pomoTicks >= pomoLengthMin*60) { isPomodoroBreak=true; pomoTicks=0; QDesktopServices::openUrl(QUrl("https://www.youtube.com/watch?v=dQw4w9WgXcQ")); /* Placeholder for break page */ }
    ...
    void syncLoop() { // 4000ms
        // Fetch License & Commands via WinInet (Preserved exactly as requested)
        ...
        else if (isLicenseValid) { lblLicense->setText(QString("PREMIUM: %1 DAYS LEFT").arg(trialDaysLeft)); lblLicense->setStyleSheet("color: green; font-weight: bold; margin-left: 30px;"); }
